...
function App() {
    ...
    useEffect(() => {
        if (db && userId && isAuthReady) {
            // Fetch Projects
            const projectsQuery = collection(db, `artifacts/${appId}/users/${userId}/projects`);
    ...
    const handleAddOrUpdateProject = async () => {
        ...
        try {
            if (editingProjectId) {
                ...
                await updateDoc(doc(db, `artifacts/${appId}/users/${userId}/projects`, editingProjectId), {
            ...
            console.error("Erro ao adicionar/atualizar projeto: ", e);
        }
    };

    const handleEditProject = (projectItem) => {
        setNewProjectTitle(projectItem.title);
        setNewProjectDescription(projectItem.description);
    ...
    if (!isAuthReady) {
        return (
            <div className="flex items-center justify-center min-h-screen bg-gray-100">
                <p className="text-lg text-gray-700">Carregando painel de gestão de projetos...</p>
            </div>
        );
    }
    ...
    return (
        <div className="container mx-auto p-4 sm:p-6 lg:p-8">
            ...
            <header className="bg-gradient-to-r from-blue-600 to-blue-800 text-white p-6 rounded-lg shadow-lg mb-8">
                <h1 className="text-3xl sm:text-4xl font-bold text-center mb-2">Painel de Gestão de Projetos</h1>
                <p className="text-center text-blue-200">Gerencie os projetos da escola</p>
            ...
            <nav className="bg-white p-4 rounded-lg shadow-md mb-8 flex flex-wrap justify-center gap-4">
                ...
                <a href="projetos.html" className="text-blue-700 hover:text-blue-900 font-semibold px-4 py-2 rounded-md transition duration-300 ease-in-out hover:bg-blue-100">Projetos (Público)</a>
            ...
            <section className="bg-white p-6 rounded-lg shadow-md mb-8">
                <h2 className="text-2xl font-bold text-blue-700 mb-4 border-b-2 border-blue-300 pb-2">Adicionar/Editar Projeto</h2>
                <div className="flex flex-col gap-4 mb-4">
                    ...
                    >
                        {editingProjectId ? 'Atualizar Projeto' : 'Adicionar Projeto'}
                    </button>
                    {editingProjectId && (
                        <button
                            onClick={() => {
                ...
                <h2 className="text-2xl font-bold text-blue-700 mb-4 border-b-2 border-blue-300 pb-2 mt-8">Projetos Existentes</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {projects.map((project) => (
                        <div key={project.id} className="bg-gray-100 p-4 rounded-lg shadow-sm flex flex-col justify-between">
                            <div>
                    ...
                    {projects.length === 0 && <p className="text-gray-500 col-span-full text-center">Nenhum projeto adicionado ainda.</p>}
                </div>
            </section>
